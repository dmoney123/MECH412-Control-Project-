...
#Nominal data conditioning, treating the entire data set as one single input-output data set with no split. Standardizes the data to have mean=0, std=1.
def no_ksplit():
    u_mean = np.mean(u_raw)  # mean input
    u_std = np.std(u_raw)   # standard deviation input
    y_mean = np.mean(y_raw) # mean output
    y_std = np.std(y_raw)   # standard deviation output
    
    # Standardize data and flip "up down"
    uk = (u_raw[::-1] - u_mean) / u_std
    yk = (y_raw[::-1] - y_mean) / y_std

    # Since no split, test data is same as training data
    uk_test = uk
    yk_test = yk

    return uk, yk, u_mean, u_std, y_mean, y_std, uk_test, yk_test
...
#This splits a given dataset into training and test, also standardizes the data.
def train_test_split():
    ...
    # Use only training data for standardization parameters
    u_mean = np.mean(u_train)
    u_std = np.std(u_train)
    y_mean = np.mean(y_train)
    y_std = np.std(y_train)

    # Standardize both train and test and flip
    uk_train = (u_train[::-1] - u_mean) / u_std
    yk_train = (y_train[::-1] - y_mean) / y_std
    uk_test = (u_test[::-1] - u_mean) / u_std
    yk_test = (y_test[::-1] - y_mean) / y_std
    
    return uk_train, yk_train, u_mean, u_std, y_mean, y_std, uk_test, yk_test
...
def cross_dataset_split(train_dataset=0):
    ...
    # Load and standardize test datasets
    test_datasets = {}
    for i in range(4):
        if i != train_dataset:
            test_data = data[i, :, :]
    ...
    return {
        ...
        'test': test_datasets,
        'scaling': {'u_mean': u_mean, 'u_std': u_std, 'y_mean': y_mean, 'y_std': y_std}
    }



# Choose your data split method:
#uk, yk, u_mean, u_std, y_mean, y_std, uk_test, yk_test = no_ksplit()  # Change this to test different splits
#uk, yk, u_mean, u_std, y_mean, y_std, uk_test, yk_test = train_test_split()  # Split single dataset by time

# Cross-dataset split: train on dataset 0, test on datasets 1, 2, 3
cross_dataset_data = cross_dataset_split(train_dataset=0)
uk = cross_dataset_data['train']['input']
yk = cross_dataset_data['train']['output']
u_mean = cross_dataset_data['scaling']['u_mean']
u_std = cross_dataset_data['scaling']['u_std']
y_mean = cross_dataset_data['scaling']['y_mean']
y_std = cross_dataset_data['scaling']['y_std']

#Inspired by lecture notes 7a-sysID# --> CONSIDER REFACTORING <---

# 1st Order System: y[k] = -a1*y[k-1] + b1*u[k-1]
def A_matrix_1stOrder(uk, yk, u_mean, u_std, y_mean, y_std):
    ...
    return A, b, u_mean, u_std, y_mean, y_std

# 2nd Order System: y[k] = -a1*y[k-1] - a2*y[k-2] + b1*u[k-1] + b2*u[k-2]
def A_matrix_2ndOrder(uk, yk, u_mean, u_std, y_mean, y_std):
...
def A_matrix_3rdOrder(uk, yk, u_mean, u_std, y_mean, y_std):
    # Form A and B matrices for system ID
    N_train = len(yk)  # Use actual length of training data
    b = yk[:-3].reshape(-1, 1)
...
def solve_Ax_b(order, uk, yk, u_mean, u_std, y_mean, y_std):

    # Get A matrix and data based on order
    if order == 1:
        A, b, u_mean, u_std, y_mean, y_std = A_matrix_1stOrder(uk, yk, u_mean, u_std, y_mean, y_std)
    elif order == 2:
        A, b, u_mean, u_std, y_mean, y_std = A_matrix_2ndOrder(uk, yk, u_mean, u_std, y_mean, y_std)
    elif order == 3:
        A, b, u_mean, u_std, y_mean, y_std = A_matrix_3rdOrder(uk, yk, u_mean, u_std, y_mean, y_std)
    else:
    ...
    # Create discrete-time transfer function DONT FORGET TO UNSTANDARDIZE
    Pd_ID = y_std / u_std * control.tf(Pd_ID_num, Pd_ID_den, T)
...
def get_models(uk, yk, u_mean, u_std, y_mean, y_std):
    models = {}
    for order in [1, 2, 3]:
        P_d, P_c = solve_Ax_b(order, uk, yk, u_mean, u_std, y_mean, y_std)
        models[order] = {'discrete': P_d, 'continuous': P_c}
    return models

def compute_models():
    models = get_models(uk, yk, u_mean, u_std, y_mean, y_std)
...
def plot_model_comparison(models, uk, yk, u_mean, u_std, y_mean, y_std, uk_test, yk_test, t):
    """
    ...
    # Unstandardize the data for plotting
    u_actual = uk[::-1] * u_std + u_mean  # Flip back and unstandardize
    y_actual = yk[::-1] * y_std + y_mean  # Flip back and unstandardize
    u_test_actual = uk_test[::-1] * u_std + u_mean
    y_test_actual = yk_test[::-1] * y_std + y_mean
...
def get_Atest(order, uk_test, yk_test, u_mean, u_std, y_mean, y_std):
    models = {}
    for order in [1, 2, 3]:
        if order == 1:
            A, b, u_mean, u_std, y_mean, y_std = A_matrix_1stOrder(uk_test, yk_test, u_mean, u_std, y_mean, y_std)
        elif order == 2:
            A, b, u_mean, u_std, y_mean, y_std = A_matrix_2ndOrder(uk_test, yk_test, u_mean, u_std, y_mean, y_std)
        elif order == 3:
            A, b, u_mean, u_std, y_mean, y_std = A_matrix_3rdOrder(uk_test, yk_test, u_mean, u_std, y_mean, y_std)
        models[order] = {'A': A, 'b': b, 'u_mean': u_mean, 'u_std': u_std, 'y_mean': y_mean, 'y_std': y_std}
    return models


#For single datase (split) validation
def compute_error(uk, yk, u_mean, u_std, y_mean, y_std, uk_test, yk_test):
    results = {}
    print("=== Model Validation Results (Standardized) ===")
    ...
    for order in [1, 2, 3]:
        ...
        if order == 1:
            A_train, b_train, u_mean, u_std, y_mean, y_std = A_matrix_1stOrder(uk, yk, u_mean, u_std, y_mean, y_std)
        elif order == 2:
            A_train, b_train, u_mean, u_std, y_mean, y_std = A_matrix_2ndOrder(uk, yk, u_mean, u_std, y_mean, y_std)
        elif order == 3:
            A_train, b_train, u_mean, u_std, y_mean, y_std = A_matrix_3rdOrder(uk, yk, u_mean, u_std, y_mean, y_std)
        ...
        test_models = get_Atest(order, uk_test, yk_test, u_mean, u_std, y_mean, y_std)
...
def compute_cross_dataset_error(cross_dataset_data):
    ...
    for order in [1, 2, 3]:
        ...
        if order == 1:
            A_train, b_train, _, _, _, _ = A_matrix_1stOrder(uk_train, yk_train, u_mean, u_std, y_mean, y_std)
        elif order == 2:
            A_train, b_train, _, _, _, _ = A_matrix_2ndOrder(uk_train, yk_train, u_mean, u_std, y_mean, y_std)
        elif order == 3:
            A_train, b_train, _, _, _, _ = A_matrix_3rdOrder(uk_train, yk_train, u_mean, u_std, y_mean, y_std)
        ...
        for test_idx, test_data in cross_dataset_data['test'].items():
            ...
            if order == 1:
                A_test, b_test, _, _, _, _ = A_matrix_1stOrder(uk_test, yk_test, u_mean, u_std, y_mean, y_std)
            elif order == 2:
                A_test, b_test, _, _, _, _ = A_matrix_2ndOrder(uk_test, yk_test, u_mean, u_std, y_mean, y_std)
            elif order == 3:
                A_test, b_test, _, _, _, _ = A_matrix_3rdOrder(uk_test, yk_test, u_mean, u_std, y_mean, y_std)
...
compute_cross_dataset_error(cross_dataset_data)
